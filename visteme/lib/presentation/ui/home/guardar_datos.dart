  ...
  Widget build(BuildContext context) {
    return Scaffold(
      drawer: BlocBuilder<AuthCubit, AuthState>(
        ...
        builder: (_, state) {
          final authEmail = (state as AuthSignedIn).user;
          return BlocBuilder<MyUserCubit, MyUserState>(
              buildWhen: (previous, current) => current is AuthSignedIn,
              builder: (_, state) {
                final authNombre = (state as MyUserReadyState).user;
                return ListView(children: [
                  UserAccountsDrawerHeader(
                    accountEmail: Text('Hotmail: ${authEmail.email}'),
                    accountName: Text('Nombre: ${authNombre.name}'
                        '${authNombre.lastName}'),
