...
import 'package:visteme/data/models/wishlist_model.dart';
    ...
    if (state is WishlistLoaded) {
      try {
        yield WishlistLoaded(
          wishlist: Wishlist(
            blusas: List.from(state.wishlist.blusas)..add(event.blusa),
          ),
        );
      } on Exception {
        yield WishlistError();
  ...
  Stream<WishlistState> _mapWishlistProductRemovedToState(
    ...
    if (state is WishlistLoaded) {
      try {
        yield WishlistLoaded(
          wishlist: Wishlist(
            blusas: List.from(state.wishlist.blusas)..remove(event.blusa),
          ),
        );
      } on Exception {
        yield WishlistError();
