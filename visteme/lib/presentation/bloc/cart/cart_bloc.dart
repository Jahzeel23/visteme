...
import 'package:visteme/data/models/cart_model.dart';
  ...
  Stream<CartState> _mapCartProductAddedToState(
    ...
    if (state is CartLoaded) {
      try {
        yield CartLoaded(
          cart: Cart(
            blusas: List.from(state.cart.blusas)..add(event.product),
          ),
        );
      } on Exception {
        yield CartError();
  ...
  Stream<CartState> _mapCartProductRemovedToState(
    ...
    if (state is CartLoaded) {
      try {
        yield CartLoaded(
          cart: Cart(
            blusas: List.from(state.cart.blusas)..remove(event.product),
          ),
        );
      } on Exception {
        yield CartError();
